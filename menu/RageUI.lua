...
function RageUI.Line(color, width, spacingTop, spacingBottom)
    local lineColor = color or {255, 255, 255, 255}
    local lineWidth = (width or 200) / 1920
    local spaceTop = spacingTop or 20
    local spaceBottom = spacingBottom or 20

    local currentMenu = RageUI.CurrentMenu
    if currentMenu ~= nil then
        RageUI.ItemOffset = RageUI.ItemOffset + spaceTop

        local menuWidth = currentMenu.WidthOffset + 431
        local x = currentMenu.X + (menuWidth / 2) / 1920
        local y = currentMenu.Y + currentMenu.SubtitleHeight / 1080 + RageUI.ItemOffset / 1080

        DrawRect(x, y, lineWidth, 0.0025, lineColor[1], lineColor[2], lineColor[3], lineColor[4])

        RageUI.ItemOffset = RageUI.ItemOffset + spaceBottom
    end
end